...
const GameBoard: React.FC<GameBoardProps> = ({ playerName, isPlacingShips, onShipsPlaced }) => {
    const BOARD_SIZE = 10;
    ...
    const canPlaceShip = (row: number, col: number, ship: Ship | null) => {
        ...
        if (shipOrientation === 'horizontal' && col + ship.size > BOARD_SIZE) return false;
        if (shipOrientation === 'vertical' && row + ship.size > BOARD_SIZE) return false;
    ...
    const placeShip = (row: number, col: number) => {
        ...
        const alreadyPlacedIndex = newPlacedShips.findIndex(ship => ship.name === selectedShip.name);
        ...
        newPlacedShips.push({
            ...selectedShip,
            positions: Array(selectedShip.size).fill(null).map((_, i) => ({
    ...
    return (
        <div className="p-4">
                ...
                <div className="mb-6">
                    <div className="flex flex-wrap gap-4 mb-4">
                        {remainingShips.map((ship) => (
                            <button
                                key={ship.name}
                                className={`px-4 py-2 rounded ${selectedShip?.name === ship.name ? 'ring-2 ring-blue-600' : 'bg-gray-200'}`}
                ...
                {board.map((row, rowIndex) => (
                    <div key={rowIndex} className="flex">
                        ...
                        {row.map((cell, colIndex) => (
                            <div
                                key={colIndex}
                                className={`w-8 h-8 border border-gray-400 ${cell ? cell.color : 'bg-white'} 
                  ${isPlacingShips && selectedShip && canPlaceShip(rowIndex, colIndex, selectedShip)
            ...
            <div className="mt-4">
                <p>Ships placed: {placedShips.length}/{SHIP_TYPES.length}</p>
            </div>
        </div>
    );
};
